...
<button class="item__button item__button--${type}" data-goto=${
  type === "prev" ? page - 1 : page + 1
}>
<i class="item__button__icon far fa-arrow-alt-circle-${
...
export const higlihght = id => {
  try {
    if (document.querySelector(".item__items--active")) {
      document
        .querySelector(".item__items--active")
        .classList.remove("item__items--active");
    }
    document
      .querySelector(`.item__search a[href*="${id}"]`)
      .classList.add("item__items--active");
  } catch (error) {
    console.log();
  }
};
