...
const likeSearch = () => {
    ...
    if (!state.heart.isHeart(id)) {
        //add state
        const newHeart = state.heart.addHeart(id, state.recipe.image, state.recipe.title, state.recipe.categories, state.recipe.rating, state.recipe.review);
...
const reserveRestaurant = async () => {
  ...
  const item = state.reserved.addItem(state.recipe.image, state.recipe.title, state.recipe.street, state.recipe.city, state.recipe.date, state.recipe.hours);
...
elements.detail.addEventListener('click', e => {
    const id = state.recipe.id;
    if (e.target.matches('.detail__reservation__minus, detail__reservation__minus *')) {
if (state.recipe.people > 1) {
        state.recipe.updatePeople('dec')
        recipeView.people(state.recipe);
}
    }else if (e.target.matches('.detail__reservation__plus, detail__reservation__plus *')) {
        ...
        likeSearch();
    }else if (e.target.matches('.detail__reservation__input, .detail__reservation__input *')) {

        ////URADI OVO !!!
        const val = e.target.value;
        state.recipe.updateDate(val);
       
    }else if (e.target.matches('.detail__reservation__hours__select, .detail__reservation__hours__select *')) {
        const val = e.target.value;
...
elements.card.addEventListener('click', e => {
    ...
    if (e.target.matches('.reserved__card__new__button, .reserved__card__new__button *')) {
